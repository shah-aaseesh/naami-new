...
export const LogoScrollIntro: React.FC<LogoScrollIntroProps> = ({ onComplete, isCompleted = false }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const [skipped, setSkipped] = useState(isCompleted);

  // Track scroll progress within the intro container (0 to 1)
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ['start start', 'end end'],
  });

  // Snappy spring so animation tracks scroll immediately
  ...
  // Full zoom burst — completes within the short ~120vh window
  const logoScale = useTransform(smoothProgress, [0, 0.5, 1], [1, 18, 80]);
  
  // Logo fades out quickly in the second half of scroll
  const logoOpacity = useTransform(smoothProgress, [0, 0.45, 0.75, 1], [1, 1, 0.2, 0]);
  
  // Scroll hint disappears almost immediately
  const promptOpacity = useTransform(smoothProgress, [0, 0.08], [1, 0]);
  const promptY = useTransform(smoothProgress, [0, 0.08], [0, 20]);

  // Background curtain reveals fast
  const overlayOpacity = useTransform(smoothProgress, [0, 0.5, 0.8, 1], [1, 1, 0.1, 0]);

  // Monitor scroll completion to inform parent
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [smoothProgress, onComplete, skipped]);
  ...
  return (
    <div ref={containerRef} className="relative h-[120vh] w-full z-40">
      {/* Sticky full-screen intro overlay */}
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center select-none pointer-events-none">
        
        {/* Dynamic Background Curtain */}
        <motion.div
          className="absolute inset-0 bg-[#FAF8F5] z-10 pointer-events-none"
          style={{ opacity: overlayOpacity }}
        />

        {/* Ambient subtle glow ring behind logo */}
        <motion.div
          className="absolute w-[450px] h-[450px] rounded-full bg-[#CC030E]/5 blur-3xl pointer-events-none z-0"
          style={{
            scale: useTransform(smoothProgress, [0, 0.8], [1, 4]),
            opacity: useTransform(smoothProgress, [0, 0.5, 0.8], [0.6, 0.8, 0]),
          }}
        />

        {/* Centered Logo Zoom Container */}
        <motion.div
          className="relative z-20 w-[280px] h-[280px] sm:w-[360px] sm:h-[360px] md:w-[420px] md:h-[420px] flex items-center justify-center"
          style={{
            scale: logoScale,
            opacity: logoOpacity,
            transformOrigin: '50% 42%', // Focus zooming through the upper-center emblem hollow space
          }}
        >
          <svg
        ...
        {/* Floating Controls & Guidance Prompts */}
        <motion.div
          className="absolute bottom-12 z-30 flex flex-col items-center gap-3 pointer-events-auto"
          style={{ opacity: promptOpacity, y: promptY }}
        >
          <div className="flex items-center gap-2 text-xs uppercase tracking-[0.25em] text-[#666] font-semibold bg-white/80 backdrop-blur-md px-5 py-2.5 rounded-full border border-black/5 shadow-sm">
            <span>Scroll to Enter Homepage</span>
            <motion.div
              animate={{ y: [0, 5, 0] }}
              transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
            >
              <ChevronDown className="w-4 h-4 text-[#CC030E]" />
            </motion.div>
          </div>
        </motion.div>
        ...
        <motion.button
          onClick={handleSkip}
          className="absolute top-8 right-8 z-30 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-[#555] hover:text-[#CC030E] bg-white/80 hover:bg-white backdrop-blur-md border border-black/5 rounded-full shadow-sm transition-all duration-200 cursor-pointer pointer-events-auto flex items-center gap-1.5"
